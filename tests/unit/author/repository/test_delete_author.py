...
class TestDeleteAuthor(AuthorRepositoryConftest):
    ...
    def test_delete_existing_author(self):
        ...
        self.author_write_repository.delete_author(id=str(author.id))

        # Assert - Author should no longer exist
        with self.assertRaises(NotFoundException):
            self.author_read_repository.get_author_by_id(id=author.id)
    ...
    def test_delete_non_existent_author(self):
        ...
        with self.assertRaises(NotFoundException):
            self.author_read_repository.get_author_by_id(id=uuid7())
